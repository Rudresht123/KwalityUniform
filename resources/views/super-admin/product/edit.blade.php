...
<style>
    ...
    .variant-row .form-control, .variant-row .form-select {
        border-color: #e2e8f0;
        background: #fff;
    }
...
<div class="container-fluid pb-5">
    <form action="{{ route('product.update', $product->product_id) }}" method="POST" enctype="multipart/form-data" id="product-form">
        @csrf
        @method('PUT')
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                <h4 class="fw-bold mb-1">Edit Product: {{ $product->product_code }}</h4>
                <p class="text-muted fs-13">Update your product details, gallery, and variants.</p>
            </div>
            <div class="btn-list">
        ...
        <div class="row">
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="product_code" value="{{ old('product_code', $product->product_code) }}" 
                                       class="form-control form-control-sm @error('product_code') is-invalid @enderror" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="product_name" value="{{ old('product_name', $product->product_name) }}" 
                                       class="form-control form-control-sm @error('product_name') is-invalid @enderror" required>
                    ...
                    <div class="card-body px-4 pb-4">
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="gender_type" class="form-select form-select-sm @error('gender_type') is-invalid @enderror" required>
            ...
            <div class="col-xl-4">
                    ...
                    <div class="card-body px-4 pb-4">
                        <div class="row g-4">
                            ...
                            <div class="col-md-12">
                                ...
                                <select name="approval_status" class="form-select form-select-sm @error('approval_status') is-invalid @enderror" required>
                ...
                <div class="card form-section-card">
                    ...
                    <div class="card-body px-4 pb-4">
                        ...
                        <div class="media-upload-wrapper" id="media-preview-container" style="grid-template-columns: repeat(auto-fill, minmax(100px, 1fr));">
                            @foreach($product->images as $image)
                                <div class="preview-item">
                                    <img src="{{ $image->url }}">
                                    @if($image->is_primary)
                                        <span class="badge bg-primary position-absolute bottom-0 start-0 m-2 fs-10">PRIMARY</span>
                                    @endif
                                </div>
                            @endforeach
                            <label class="image-upload-box" id="upload-trigger">
                                ...
                                <input type="file" name="images[]" id="images-input" multiple class="d-none" accept="image/*">
                                <div class="upload-animation" id="global-upload-bar"></div>
                            </label>
                        </div>
                        @error('images') <div class="text-danger fs-12 mt-2">{{ $message }}</div> @enderror
                    </div>
                </div>
            </div>
            ...
            <div class="col-12">
                <div class="card form-section-card">
                    <div class="card-header d-flex align-items-center justify-content-between border-bottom-0 pt-4 px-4">
                                ...
                                <tbody>
                                    ...
                                    @foreach($variants as $index => $variant)
                                        <tr class="variant-row">
                                            <input type="hidden" name="variants[{{ $index }}][variant_id]" value="{{ is_array($variant) ? ($variant['variant_id'] ?? '') : $variant->variant_id }}">
                                            <td><input type="text" name="variants[{{ $index }}][sku]" class="form-control form-control-sm" value="{{ is_array($variant) ? $variant['sku'] : $variant->sku }}" required></td>
                                            <td>
                                                <select name="variants[{{ $index }}][size_id]" class="form-select form-select-sm">
                                                    <option value="">N/A</option>
                                                    @foreach($sizes as $size)
                                                        <option value="{{ $size->size_id }}" {{ (is_array($variant) ? $variant['size_id'] : $variant->size_id) == $size->size_id ? 'selected' : '' }}>{{ $size->size_name }}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td>
                                                <select name="variants[{{ $index }}][color_id]" class="form-select form-select-sm">
                                                    ...
                                                    @foreach($colors as $color)
                                                        <option value="{{ $color->color_id }}" {{ (is_array($variant) ? $variant['color_id'] : $variant->color_id) == $color->color_id ? 'selected' : '' }}>{{ $color->color_name }}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td><input type="number" step="0.01" name="variants[{{ $index }}][mrp]" class="form-control form-control-sm" value="{{ is_array($variant) ? $variant['mrp'] : $variant->mrp }}" required></td>
                                            <td><input type="number" step="0.01" name="variants[{{ $index }}][selling_price]" class="form-control form-control-sm" value="{{ is_array($variant) ? $variant['selling_price'] : $variant->selling_price }}" required></td>
                                            <td><input type="number" name="variants[{{ $index }}][stock_qty]" class="form-control form-control-sm" value="{{ is_array($variant) ? $variant['stock_qty'] : $variant->stock_qty }}" required></td>
                                            <td><input type="number" name="variants[{{ $index }}][low_stock_alert]" class="form-control form-control-sm" value="{{ is_array($variant) ? ($variant['low_stock_alert'] ?? 5) : $variant->low_stock_alert }}"></td>
                                            <td><input type="text" name="variants[{{ $index }}][barcode]" class="form-control form-control-sm" value="{{ is_array($variant) ? ($variant['barcode'] ?? '') : $variant->barcode }}"></td>
                                            <input type="hidden" name="variants[{{ $index }}][is_active]" value="1">
                                            <td class="text-center">
                                                <div class="variant-action-group">
                                                    <button type="button" class="btn-variant btn-variant-danger remove-variant" title="Remove Row"><i class="ti ti-trash"></i></button>
                                                    <button type="button" class="btn-variant btn-variant-primary add-variant-btn" title="Add Row Below"><i class="ti ti-plus"></i></button>
                                                </div>
                                            </td>
                                            </tr>
                                            @endforeach
                                            ...
                                            <td><input type="text" name="variants[${variantIndex}][sku]" class="form-control form-control-sm shadow-none" placeholder="e.g. SKU-01" required></td>
                                            <td>
                                            <select name="variants[${variantIndex}][size_id]" class="form-select form-select-sm shadow-none">
                                            <option value="">N/A</option>
                                            @foreach($sizes as $size) <option value="{{ $size->size_id }}">{{ $size->size_name }}</option> @endforeach
                                            </select>
                                            </td>
                                            <td>
                                            <select name="variants[${variantIndex}][color_id]" class="form-select form-select-sm shadow-none">
                                            <option value="">N/A</option>
                                            @foreach($colors as $color) <option value="{{ $color->color_id }}">{{ $color->color_name }}</option> @endforeach
                                            </select>
                                            </td>
                                            <td><input type="number" step="0.01" name="variants[${variantIndex}][mrp]" class="form-control form-control-sm shadow-none" placeholder="0.00" required></td>
                                            <td><input type="number" step="0.01" name="variants[${variantIndex}][selling_price]" class="form-control form-control-sm shadow-none" placeholder="0.00" required></td>
                                            <td><input type="number" name="variants[${variantIndex}][stock_qty]" class="form-control form-control-sm shadow-none" placeholder="0" required></td>
                                            <td><input type="number" name="variants[${variantIndex}][low_stock_alert]" class="form-control form-control-sm shadow-none" value="5" placeholder="5"></td>
                                            <td><input type="text" name="variants[${variantIndex}][barcode]" class="form-control form-control-sm shadow-none" placeholder="UPC/EAN"></td>
